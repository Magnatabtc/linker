...
func NewProvidersWizard(in io.Reader, out io.Writer, store *config.Store, repo *state.Repository, providers *provider.Registry, catalog *catalog.Service, defaultCfg config.Config) *ProvidersWizard {
	return &ProvidersWizard{
...
func (w *ProvidersWizard) Run(ctx context.Context) (config.Config, error) {
	cfg, err := w.store.Load(w.defaultCfg)
	if err != nil {
		return config.Config{}, err
	}

	fmt.Fprintln(w.out, styleHeader.Render("Configure Providers"))
	fmt.Fprintln(w.out, "  Connect or manage OAuth/API-key providers without touching model mappings.")
	...
	if _, err := w.catalog.Refresh(ctx, cfg); err != nil {
		fmt.Fprintf(w.out, "%s\n", styleWarning.Render("! Provider setup saved, but catalog refresh failed: "+err.Error()))
	}

	fmt.Fprintln(w.out, styleSuccess.Render("✔ Provider configuration saved."))
...
func (w *ProvidersWizard) providerSelectionFlow(cfg config.Config) ([]string, error) {
	...
	choice, err := w.instantSelect(
		"Provider Setup",
		"Keep, add, remove, or fully replace enabled providers.",
...
func (w *ProvidersWizard) configureSelectedProviders(ctx context.Context, cfg config.Config, selectedProviders []string) (map[string]config.ProviderConfig, error) {
...
func (w *ProvidersWizard) configureProviderAuth(ctx context.Context, providerID string, existingProvider config.ProviderConfig) (config.ProviderConfig, error) {
...
func (w *ProvidersWizard) configureOAuthProvider(ctx context.Context, providerID string, existingProvider config.ProviderConfig) (config.ProviderConfig, error) {
	...
	for {
		...
		choice, err := w.instantSelect(
			"Account Management",
			"Manage provider accounts without changing model mappings.",
...
func (w *ProvidersWizard) captureAccount(ctx context.Context, providerID string, existing *state.AccountAuth) (state.AccountAuth, string, provider.Info, error) {
...
func (w *ProvidersWizard) selectProviders(cfg config.Config) ([]string, error) {
	...
	err := huh.NewForm(
		huh.NewGroup(
			huh.NewMultiSelect[string]().
				Title("AI Providers").
				Description("Choose providers to enable. Use Space to select, Enter to confirm.").
				Options(options...).
				Value(&selected),
		),
	).Run()
...
func (w *ProvidersWizard) addProviders(cfg config.Config) ([]string, error) {
...
func (w *ProvidersWizard) removeProviders(cfg config.Config) ([]string, error) {
	current := currentProviderIDs(cfg)
	if len(current) == 0 {
		return w.selectProviders(cfg)
	}
...
func (w *ProvidersWizard) ask(title string, fallback string) string {
	var result string
	err := huh.NewForm(
		huh.NewGroup(
			huh.NewInput().
...
func (w *ProvidersWizard) askConfirm(title string, defaultValue bool) bool {
	var result bool
	err := huh.NewForm(
		huh.NewGroup(
			huh.NewConfirm().
...
func (w *ProvidersWizard) chooseAccountIndex(prompt string, accounts []config.AccountRef, activeID string) int {
	fallback := "1"
...
func (w *ProvidersWizard) instantSelect(title string, description string, options []string) (int, error) {
	m := selectionModel{title: title, description: description, options: options, choice: -1}
	p := tea.NewProgram(m)
...
func (w *ProvidersWizard) providerSummaryLines(cfg config.Config) []string {
	lines := []string{}
	for _, providerID := range currentProviderIDs(cfg) {
...
func (w *ProvidersWizard) existingProviderAuth(providerCfg config.ProviderConfig) *state.AccountAuth {
...
func (w *ProvidersWizard) loadAccountAuth(account config.AccountRef) *state.AccountAuth {
	if account.AuthFile == "" {
		return nil
	}
...
func resolveAuthPath(root string, value string) string {
	if value == "" {
		return ""
	}
